...
// Advanced Audio Controls
function initAdvancedAudioControls() {
    const advancedToggle = document.getElementById('advancedAudioToggle');
    const advancedPanel = document.getElementById('advancedAudioPanel');
    
    // Toggle advanced panel
    advancedToggle?.addEventListener('click', () => {
        advancedPanel?.classList.toggle('hidden');
        advancedToggle?.classList.toggle('active');
        window.analytics?.track('audio_advanced_toggle', {
            state: advancedPanel?.classList.contains('hidden') ? 'closed' : 'open'
        });
    });
    
    // Crackling controls
    const cracklingEnabled = document.getElementById('cracklingEnabled');
    const cracklingIntensity = document.getElementById('cracklingIntensity');
    const cracklingIntensityValue = document.getElementById('cracklingIntensityValue');
    const cracklingPitch = document.getElementById('cracklingPitch');
    const cracklingPitchValue = document.getElementById('cracklingPitchValue');
    
    cracklingEnabled?.addEventListener('change', (e) => {
        window.audioConfig.profile.crackling.enabled = e.target.checked;
        window.analytics?.track('audio_crackling_toggle', { enabled: e.target.checked });
    });
    
    cracklingIntensity?.addEventListener('input', (e) => {
        const value = e.target.value / 100;
        window.audioConfig.profile.crackling.intensity = value;
        cracklingIntensityValue.textContent = e.target.value + '%';
    });
    
    cracklingPitch?.addEventListener('input', (e) => {
        const value = e.target.value / 100;
        window.audioConfig.profile.crackling.pitch = value;
        const labels = ['Very Low', 'Low', 'Medium', 'High', 'Very High'];
        const index = Math.floor(value * 4);
        cracklingPitchValue.textContent = labels[index];
    });
    
    // Explosion controls
    const explosionBass = document.getElementById('explosionBass');
    const explosionBassValue = document.getElementById('explosionBassValue');
    const explosionDuration = document.getElementById('explosionDuration');
    const explosionDurationValue = document.getElementById('explosionDurationValue');
    
    explosionBass?.addEventListener('input', (e) => {
        const value = e.target.value / 100;
        window.audioConfig.profile.explosion.bass = value;
        const labels = ['Very Low', 'Low', 'Medium', 'High', 'Very High'];
        const index = Math.floor(value * 4);
        explosionBassValue.textContent = labels[index];
    });
    
    explosionDuration?.addEventListener('input', (e) => {
        const value = e.target.value / 100;
        window.audioConfig.profile.explosion.duration = value;
        const labels = ['Very Short', 'Short', 'Medium', 'Long', 'Very Long'];
        const index = Math.floor(value * 4);
        explosionDurationValue.textContent = labels[index];
    });
    
    // Launch controls
    const launchPitch = document.getElementById('launchPitch');
    const launchPitchValue = document.getElementById('launchPitchValue');
    const launchDuration = document.getElementById('launchDuration');
    const launchDurationValue = document.getElementById('launchDurationValue');
    
    launchPitch?.addEventListener('input', (e) => {
        const value = e.target.value / 100;
        window.audioConfig.profile.launch.pitch = value;
        const labels = ['Very Low', 'Low', 'Medium', 'High', 'Very High'];
        const index = Math.floor(value * 4);
        launchPitchValue.textContent = labels[index];
    });
    
    launchDuration?.addEventListener('input', (e) => {
        const value = e.target.value / 100;
        window.audioConfig.profile.launch.duration = value;
        const labels = ['Very Short', 'Short', 'Medium', 'Long', 'Very Long'];
        const index = Math.floor(value * 4);
        launchDurationValue.textContent = labels[index];
    });
    
    // Profile management
    const saveProfileBtn = document.getElementById('saveAudioProfile');
    const profileNameInput = document.getElementById('audioProfileName');
    const loadProfileSelect = document.getElementById('loadAudioProfile');
    const resetProfileBtn = document.getElementById('resetAudioProfile');
    
    saveProfileBtn?.addEventListener('click', () => {
        const name = profileNameInput.value.trim();
        if (name) {
            window.saveAudioProfile(name);
            profileNameInput.value = '';
            updateAudioProfileList();
            window.analytics?.track('audio_profile_save', { name_length: name.length });
            showMessage('Audio profile saved!');
        }
    });
    
    loadProfileSelect?.addEventListener('change', (e) => {
        const name = e.target.value;
        if (name && window.loadAudioProfile(name)) {
            updateAdvancedAudioUI();
            window.analytics?.track('audio_profile_load', { name_length: name.length });
            showMessage('Audio profile loaded!');
        }
    });
    
    resetProfileBtn?.addEventListener('click', () => {
        window.resetAudioProfile();
        updateAdvancedAudioUI();
        window.analytics?.track('audio_profile_reset');
        showMessage('Audio profile reset to preset!');
    });
...
function updateAudioProfileList() {
    const loadProfileSelect = document.getElementById('loadAudioProfile');
    if (!loadProfileSelect) return;
    
    const profiles = window.getSavedAudioProfiles();
    loadProfileSelect.innerHTML = '<option value="">Load saved profile...</option>';
...
function updateAdvancedAudioUI() {
    const profile = window.audioConfig.profile;
    
    // Crackling
    const cracklingEnabled = document.getElementById('cracklingEnabled');
    if (cracklingEnabled) cracklingEnabled.checked = profile.crackling.enabled !== false;
    
    if (profile.crackling.intensity !== null) {
        const slider = document.getElementById('cracklingIntensity');
        const value = document.getElementById('cracklingIntensityValue');
        if (slider) slider.value = profile.crackling.intensity * 100;
        if (value) value.textContent = Math.round(profile.crackling.intensity * 100) + '%';
    }
    
    if (profile.crackling.pitch !== null) {
        const slider = document.getElementById('cracklingPitch');
        const value = document.getElementById('cracklingPitchValue');
        if (slider) slider.value = profile.crackling.pitch * 100;
        if (value) {
            const labels = ['Very Low', 'Low', 'Medium', 'High', 'Very High'];
            const index = Math.floor(profile.crackling.pitch * 4);
            value.textContent = labels[index];
        }
    }
    
    // Explosion
    if (profile.explosion.bass !== null) {
        const slider = document.getElementById('explosionBass');
        const value = document.getElementById('explosionBassValue');
        if (slider) slider.value = profile.explosion.bass * 100;
        if (value) {
            const labels = ['Very Low', 'Low', 'Medium', 'High', 'Very High'];
            const index = Math.floor(profile.explosion.bass * 4);
            value.textContent = labels[index];
        }
    }
    
    if (profile.explosion.duration !== null) {
        const slider = document.getElementById('explosionDuration');
        const value = document.getElementById('explosionDurationValue');
        if (slider) slider.value = profile.explosion.duration * 100;
        if (value) {
            const labels = ['Very Short', 'Short', 'Medium', 'Long', 'Very Long'];
            const index = Math.floor(profile.explosion.duration * 4);
            value.textContent = labels[index];
        }
    }
    
    // Update reverb slider too
    const reverbSlider = document.getElementById('reverbSlider');
    const reverbValue = document.getElementById('reverbValue');
    if (reverbSlider && window.audioConfig.reverbAmount !== undefined) {
...
function showMessage(text) {
    // Reuse share message element if available
    const shareMessage = document.getElementById('shareMessage');
    if (shareMessage) {
